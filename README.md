...
The [`h-native-research-v2` construction and calibration protocol design](RESEARCH_CORPUS_V2_DESIGN.md) is supported by candidate tooling and standalone stronger-ISD baseline infrastructure at this stage. `rlmw_research_corpus_v2.py` provides deterministic candidate-generation and validation tooling, including deterministic pre-audit records and structurally accepted candidate records; only a complete 192-case accepted layout can become calibration-ready. No generated or frozen v2 manifest, calibrated thresholds, calibration results, final-evaluation data, replicated experiments, or neural/RL results exist yet. The stronger solver-disabled Stern/Dumer-style collision ISD v2 tooling now exists as standalone infrastructure for the next calibration stage; PR C frozen-manifest review, later replicated classical evaluation, and neural/RL evaluation remain deferred to later staged PRs.

## Executable artifacts

The primary interactive artifact is `rlmw.ipynb`. The explicitly requested standalone research tools are `rlmw_research_corpus.py`, `rlmw_research_baselines.py`, `rlmw_research_corpus_v2.py`, and `rlmw_research_isd_v2.py`; they provide reproducible v1 manifest/baseline contracts, v2 candidate-generation/validation tooling, and v2 solver-disabled Stern/Dumer ISD tooling used by CI and Colab. Neural and hybrid-search core logic remains in the notebook.
