...
For biregular cases, create `d_v` variable sockets for each coordinate and `d_c` check sockets for each row. Because `n d_v = r d_c`, both socket sets have equal size. For attempt `a`, assign every possible variable-socket/check-socket pair an independently keyed 256-bit priority using purpose `sparse_edge_priority` and socket identity `("socket", variable_index, variable_socket_index, check_index, check_socket_index)`. Process pairs in lexicographic order of `(priority, variable_index, variable_socket_index, check_index, check_socket_index)` and progressively build the Tanner graph.

An edge `(v, q)` is accepted only if the selected variable socket and check socket are both unused, column `v` is not already adjacent to check `q` (parallel-edge predicate), and adding `(v, q)` does not close a 4-cycle. The exact 4-cycle predicate is: reject `(v, q)` if there exists another variable `v2 != v` and another check `q2 != q` such that the current partial graph already contains `(v, q2)`, `(v2, q)`, and `(v2, q2)`. Equivalently, reject if the current neighbor set of `v` intersects the neighbor set of any variable already adjacent to `q`. If the ordered scan cannot consume every variable and check socket while preserving exact variable degree 3 and check degree 6, reject the construction attempt and restart with the next keyed `construction_attempt`. The public matrix entry `H[row,column]` is the parity of accepted edges; parallel edges are rejected before parity cancellation can occur. Degree, rank, girth, duplicate, and audit requirements are never relaxed.
...
Each stratum has `max_attempts = 20000`. Exhausting attempts without an accepted candidate is a structured generation failure for that stratum and must be reviewed; it must not silently relax degree, girth, rank, or duplicate requirements. PR B must include feasibility tests that deterministically generate at least one valid matrix from every sparse stratum within the frozen attempt cap, plus independent validator tests for variable degrees, check degrees, simplicity, GF(2) rank, and absence of 4-cycles.
...
3. For planted sparse strata, construct a constrained simple `(3,6)` biregular Tanner graph using the same progressive no-4-cycle socket insertion rule as Section 4.2. The planted witness support has `w_p` variables and therefore `3*w_p` witness variable sockets. For `w_p=10`, choose 15 distinct check rows by ranking checks with purpose `planted_sparse_witness_check_priority`, and allocate exactly two witness edges to each chosen check. For `w_p=12`, choose 18 distinct checks the same way and allocate exactly two witness edges to each chosen check. All other checks receive zero witness edges. Witness variable sockets are ordered by purpose `planted_sparse_witness_socket_priority`; chosen checks and their two reserved check sockets are ordered by purpose `planted_sparse_witness_check_socket_priority`. Insert witness edges in that deterministic order, accepting an edge only if it uses unused sockets, does not create a parallel edge, and does not close a 4-cycle under the Section 4.2 predicate. Then fill all remaining check sockets with non-witness variable sockets ordered by purpose `planted_sparse_nonwitness_edge_priority`, again rejecting any edge that uses an already used socket, creates a parallel edge, or closes a 4-cycle. This preserves variable degree 3 and check degree 6 exactly; every check has an even number of witness neighbors, so `Hc=0`. Ties in all priorities are broken lexicographically by the full socket identity. If any witness socket, non-witness socket, or check socket cannot be consumed, or if exact `Hc=0`, target row rank, no-zero/repeated-column, no-4-cycle, duplicate, or small-circuit checks fail, reject the construction attempt and retry with the next `construction_attempt`. The planted sparse provenance records witness support, witness-check set, socket ordering digests, attempt number, and public/evaluator-only permutation metadata.
...
Split assignment is performed before calibration and never changed based on calibration, thresholds, neural/RL results, or convenience. For each complete lineage group, compute `split_key = SHA256(encode(("split_key", protocol_id, parameter_stratum_id, lineage_group_id)))` using the Section 2 typed tuple/string encoding and sort groups lexicographically inside each parameter stratum, not merely inside the broad family class. Assign whole groups to meet exact per-stratum targets: every dense and sparse 18-case stratum has 10 train / 4 validation / 4 test; every planted 8-case stratum has 4 train / 2 validation / 2 test; exact controls allocate four complete control strata to train, two to validation, and two to test. These per-stratum targets imply the overall 104/44/44 totals in Section 6.1. Because every group has size 2, validators must check that every per-stratum split target and every overall split target is divisible by 2. If exact targets cannot be met with accepted complete groups, generation continues with the next deterministic batch; if the frozen allocation cannot be satisfied, generation fails structurally. Post-hoc movement across splits is prohibited.
...
PR B must freeze either the literal public calibration seed sets below or a fully deterministic derivation that reproduces them, plus generator/config digest, final-evaluation seed commitments, candidate-tool dependency versions or containers, and candidate-tool deterministic execution settings before candidate generation begins. The subsequent stronger-baseline PR must freeze baseline versions, baseline parameters, dependency versions or containers, thread counts, CP-SAT deterministic-time settings, and operation-count definitions before external calibration begins. PR C occurs only after calibration and must not retroactively choose seeds, budgets, algorithms, deterministic settings, threshold rules, or tier rules.
...
- `final_eval_seed[0..7]`: reserved for later replicated evaluation, never used for W selection or tier acceptance, and protected by commitment-and-reveal for leakage-protected neural/RL evaluation.

Threshold-fit and tier-validation seed bytes are derived with the Section 2 primitive using purpose `calibration_seed`, case slot `0`, construction attempt `0`, and logical identity `("seed_role", role, index)`; their actual 128-bit hex values are frozen publicly as PR B test vectors. Final-evaluation seeds use commitment-and-reveal: before training, PR B or the later final-evaluation protocol commits `SHA256(encode(("final_eval_seed_commitment", protocol_id, index, seed_bytes)))` and the deterministic derivation protocol, while the actual seed bytes remain evaluator-only. After the frozen final evaluation is complete, the evaluator reveals the seed bytes and verifies them against the commitments. Revealed final seeds must not be used for model selection, threshold selection, tier assignment, debugging, or rerunning a tuned evaluation. If a later protocol intentionally publishes final seeds before evaluation, it must label the result as a public fixed-seed benchmark rather than leakage-protected evaluation and limit claims accordingly. The CP-SAT reference uses exactly four public calibration seed-role/index pairs: `threshold_fit_seed[0]`, `threshold_fit_seed[1]`, `tier_validation_seed[0]`, and `tier_validation_seed[1]`; these provide four deterministic runs per case for each CP-SAT time-limit stratum and are reported only as solver-assisted reference evidence.
...
2. **PR B:** candidate v2 generator, validator, independent tests, RNG test vectors, public calibration seed values, final-evaluation seed commitments, deterministic execution settings, and candidate-manifest tooling only. Generated candidates remain uncommitted.
3. **Stronger-baseline PR:** implement and test at least one pinned stronger solver-disabled ISD baseline, define deterministic operation accounting and budgets, freeze implementation/dependency versions and parameters, independently verify every incumbent against public `H`, and keep solver-assisted CP-SAT results separate.
4. **External generation and calibration phase:** candidate generation, structural audits, and calibration runs occur outside GitHub only after PR B and the stronger-baseline PR are reviewed and merged; outputs remain uncommitted but are bound to source commits and digests.
5. **PR C:** evidence-backed review of acceptance decisions plus frozen v2 manifest/protocol/digest if accepted. PR C may accept or reject cases under frozen rules; it must not retroactively choose seeds, rules, budgets, or algorithms.
6. **Later PR:** replicated classical evaluation on the frozen manifest using the final-evaluation commitment/reveal process.
7. **Only afterward:** controlled neural/RL comparison, and only if the stronger solver-disabled ISD baseline requirement is satisfied.

After PR B, RNG serialization, generator version, stratum definitions, validator checks, public calibration seed derivation, final-evaluation seed commitments, and candidate-tooling deterministic execution settings are immutable for that candidate protocol. After the stronger-baseline PR, baseline versions, baseline parameters, operation accounting, budgets, and deterministic execution settings are immutable for calibration. After PR C, accepted public H matrices, W values, split assignments, digests, run specs, and public schemas are immutable. Failed calibration leads to a new candidate protocol version, not silent mutation of cases, W, seeds, or acceptance rules.
