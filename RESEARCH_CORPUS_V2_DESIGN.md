...
| draw purpose | string from a frozen enum, for example `dense_entry`, `sparse_edge_priority`, `planted_witness_coordinate`, `planted_orthogonal_row_free_bit`, `row_operation_entry`, `coordinate_permutation_key`, `calibration_seed`, or `evaluation_seed` |
...
2. Generate candidate rows independently. For dense planted rows, draw all but the largest witness coordinate with purpose `planted_orthogonal_row_free_bit`; set the largest witness coordinate to the parity needed for row dot witness = 0. For sparse planted rows, draw row supports by independently keyed priorities outside and inside witness support, require declared row weight 6, and set one selected witness-support bit to enforce orthogonality without changing row weight; reject if impossible.
3. Public row order is generated order. Rank handling is by rejection only: accept only if exact GF(2) row rank equals `r`. Do not row-reduce the public matrix.
4. Apply an invertible row operation only if generated by purpose `row_operation_entry` and recorded as evaluator-only provenance; this operation is not used to hide a defective base matrix because the pre-transform matrix must already satisfy rank, no-zero/repeated-column, and audit checks.
5. Apply a coordinate permutation generated by sorting 256-bit purpose `coordinate_permutation_key`; record the permutation orbit in evaluator-only provenance and lineage. The public H is after this permutation, and the planted witness is permuted accordingly for evaluator verification only.
6. Validate exact `H c = 0`, `wt(c)=w_p`, no zero/repeated columns, rank, duplicate hashes, lineage isolation, and small-circuit audit. The planted witness is an upper bound only, never an exact-distance label.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| theorem/exact controls | 8 control strata in Section 5.1 | 2 | 16 | 6 | 5 | 5 | 8 |
...
For dense and sparse unknown cases, PR B must create construction batches with planned group size 3: one train-intended slot, one validation-intended slot, and one test-intended slot are generated from the same `construction_batch_id` and base seed family but independent `case_slot` values. These three slots share `lineage_group_id = SHA256("lineage" || protocol_id || family_id || parameter_stratum_id || construction_batch_id)` and must be assigned to the same split if any derived transformation, permutation orbit, or rejection batch links them. If this grouping would prevent exact target counts, the entire group is discarded and a new deterministic batch is tried; individual cases are not moved post hoc.

For exact controls, the two variants in each control stratum share one lineage group. For planted cases, all variants derived from the same unpermuted planted witness and base orthogonal-row candidate share one lineage group. Planned planted groups have size 2; exact target counts are reached by deterministic acceptance of complete groups only.

### 7.3 Deterministic split assignment

Split assignment is performed before calibration and never changed based on calibration, thresholds, neural/RL results, or convenience. For each complete lineage group, compute `split_key = SHA256("split" || protocol_id || lineage_group_id)` and sort groups lexicographically by `split_key` within each stratum class. Assign groups greedily to meet the exact class-level targets in Section 6.1, preferring train, then validation, then test only when a group fits the remaining target count for that class. If exact targets cannot be met with accepted complete groups, generation continues with the next deterministic batch. Post-hoc movement across splits is prohibited.

All matrices derived from the same latent base matrix, seed lineage, planted witness, transformation orbit, coordinate permutation orbit produced by the generator, or rejection batch must remain in one split.
...
The seed bytes are derived with the Section 2 primitive using purpose `calibration_seed` or `evaluation_seed`, case slot `0`, construction attempt `0`, and logical identity `("seed_role", role, index)`. The actual resulting 128-bit hex values must be frozen as PR B test vectors.
...
| `hard_calibrated` | maximum-budget hit rate in [0.10, 0.35); verified upper bound exists; resource-limit frequency < 0.50; certified lower-bound gap explicitly accepted |
| `calibration_incomplete` | structural checks pass but fitting or tier evidence is insufficient |

A case is rejected or separately labelled when it is structurally trivial, maximum-budget hit rate is below 0.10, all baselines saturate it at the smallest budget with hit rate >= 0.90, no usable verified upper bound is obtained, construction artifacts dominate its solution, resource-limit frequency exceeds the tier limit, algorithms/repetitions disagree beyond tier limits, fitting availability is below 50%, or evidence is too weak for the intended tier. Bound gaps, solver-disabled upper bounds, and solver-assisted upper bounds must remain separate fields.
...
| insufficient statistical power | use 192 cases with train/validation/test splits and replicated seeds | still not enough for all fine-grained neural/RL claims |
