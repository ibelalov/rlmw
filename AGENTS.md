...
2. Maintain `rlmw_research_corpus_v2.py` as the candidate-generation, validation, deterministic test-vector, seed-commitment, and candidate-manifest tooling layer; deterministic pre-audit and structurally accepted candidate records are supported, but no generated/frozen v2 manifest or results exist yet.
3. With stronger solver-disabled Stern/Dumer-style collision ISD v2 tooling in place, keep external generation/calibration, PR C frozen-manifest review, later replicated classical evaluation, neural-quality comparisons, and RL-quality comparisons deferred until v2 is reviewed, calibrated, and frozen.
...
`rlmw_research_corpus_audit.py` is standalone classical correctness infrastructure for `h-native-research-v1`. It must not alter the frozen manifest, `rlmw_research_corpus.py`, baseline schemas, or solver payloads. Audit JSONL is generated output and must not be committed. The v1 corpus is quarantined for contract/audit regression only and must not be used for solver-superiority, neural/RL-quality, or benchmark-difficulty claims. The audit and certificate contracts are preserved as historical v1 quarantine conclusions. The separate [`h-native-research-v2` construction and calibration protocol](RESEARCH_CORPUS_V2_DESIGN.md) is supported at the tooling layer by `rlmw_research_corpus_v2.py`, which provides deterministic candidate-generation and validation tooling. It distinguishes deterministic pre-audit candidates from structurally accepted candidates; only a complete 192-case accepted layout can become calibration-ready. No generated or frozen v2 manifest, calibrated thresholds, calibration results, final-evaluation data, replicated experiments, or neural/RL results exist yet. The external v2 calibration harness is two-stage: threshold-fit plans/results are solver-disabled with no W in fit payloads and production budgets are never silently truncated, then an authoritatively replayed threshold artifact (manifest, complete fit plan, and complete fit results) gates tier/reference planning; tier artifacts and summaries likewise require complete replay evidence, and CP-SAT remains solver-assisted-only. The stronger solver-disabled Stern/Dumer-style collision ISD v2 tooling now exists as standalone infrastructure for the next calibration stage. PR C frozen-manifest review, later replicated classical evaluation, and neural/RL comparisons remain deferred to later staged PRs until v2 is reviewed, calibrated, and frozen.
